...
### H2 (`database/h2/H2Statements`, `SqlDialect.H2`, `H2SqlRenderer`)
- Quotes: `"` double quotes
- Pagination: standard `OFFSET n ROWS FETCH NEXT m ROWS ONLY`
- Upsert: `MERGE INTO ... USING (VALUES ...)` with quoted source aliases
- DDL types: `BOOLEAN`, `INTEGER`, `BIGINT`, `VARCHAR(n)`, `CLOB`, `BLOB`, `TIMESTAMP`
- Identity columns: `GENERATED BY DEFAULT AS IDENTITY`
- Metadata: `INFORMATION_SCHEMA.TABLES`, `COLUMNS`, `INDEXES`, and `INDEX_COLUMNS`
- Generated identity values are read through JDBC generated keys
...
### DM8 (`database/dm8/Dm8Statements`)
- Quotes and query rendering: Oracle-compatible `"` identifiers and pagination
- Upsert and metadata: Oracle-compatible statements with DM8 index metadata filtering
- Identity columns: DM8-native `INT IDENTITY(1,1)` / `BIGINT IDENTITY(1,1)`
...
```kotlin
enum class DBType { Mysql, Postgres, SQLite, H2, Mssql, Oracle, DM8 }
```
