...
### InsertClause
Chain methods: `cascade()`, `patch()`, `withId()`

Terminal: `execute()` → returns `KronosOperationResult` with `affectedRows` and optional `lastInsertId`
...
5. If `.withId()` is used and an identity primary key is generated by the database, attaches a typed `GeneratedKeyRequest`
6. For batch: groups by field set, creates multiple statements
