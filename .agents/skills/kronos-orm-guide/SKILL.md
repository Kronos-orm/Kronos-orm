...
Kronos 是一个基于 Kotlin 编译器插件的现代 ORM 框架，零反射、强类型、支持多数据库（MySQL、PostgreSQL、SQLite、H2、SQL Server、Oracle、DM8/达梦）。
...
plugins {
    ...
    id("com.kotlinorm.kronos-gradle-plugin") version "0.3.1"
}

dependencies {
    implementation("com.kotlinorm:kronos-core:0.3.1")
    // JDBC 包装器
    implementation("com.kotlinorm:kronos-jdbc-wrapper:0.3.1")
...
技能中的 Kronos 推荐稳定版本直接写 `0.3.1`。`kronos-docs` Markdown 的版本宏只用于 docs 源文件，不用于本使用指南。
...
```kotlin
plugins {
    id("com.kotlinorm.kronos-gradle-plugin") version "0.3.1" apply false
}

dependencies {
    implementation("com.kotlinorm:kronos-core:0.3.1")
    implementation("com.kotlinorm:kronos-logging:0.3.1") // Logcat SQL output
}
```
...
| 设置 | MySQL | PostgreSQL | SQLite | H2 | SQL Server | Oracle | DM8 |
|------|-------|------------|--------|----|------------|--------|-----|
| `VARCHAR, length = 80` | `VARCHAR(80)` | `VARCHAR(80)` | `TEXT` | `VARCHAR(80)` | `VARCHAR(80)` | `VARCHAR2(80)` | `VARCHAR2(80)` |
| `DECIMAL, length = 12, scale = 2` | `DECIMAL(12,2)` | `DECIMAL(12,2)` | `NUMERIC` | `DECIMAL(12,2)` | `DECIMAL(12,2)` | `NUMBER(12,2)` | `NUMBER(12,2)` |
...
```kotlin
dependencies {
    implementation("com.kotlinorm:kronos-logging:0.3.1")
}
```
...
脚本依赖使用 Kronos `0.3.1`，JDBC Driver 和连接池使用与数据库、JDK 匹配的最新稳定版：
...
当前完整内置方言：MySQL、PostgreSQL、SQLite、H2、SQL Server、Oracle、DM8（达梦）。

内置方言对应的 `DBType`：MySQL 为 `DBType.Mysql`，PostgreSQL 为 `DBType.Postgres`，SQLite 为 `DBType.SQLite`，H2 为 `DBType.H2`，SQL Server 为 `DBType.Mssql`，Oracle 为 `DBType.Oracle`，DM8 为 `DBType.DM8`。

DM8 在原生模式（`COMPATIBLE_MODE=0`）下使用 Oracle 兼容的查询、分页、`MERGE` 和 metadata SQL；`@PrimaryKey(identity = true)` 的 `Int` / `Long` 字段使用 DM8 原生 `INT` / `BIGINT IDENTITY(1,1)` DDL。`KronosJdbcWrapper` 可以通过 Dameng / DM DBMS JDBC metadata 自动识别 DM8，也可以显式传入 `databaseType = DBType.DM8`：

```kotlin
val wrapper = KronosJdbcWrapper(
    dataSource = dm8DataSource,
...
H2 使用 `DBType.H2`，JDBC metadata 会自动识别 H2 产品名和 URL；需要显式声明时可传入 `databaseType = DBType.H2`。H2 支持内存和文件 URL，常用的内存数据源配置如下：

```kotlin
import com.kotlinorm.enums.DBType
import com.kotlinorm.wrappers.KronosJdbcWrapper

val h2Wrapper = KronosJdbcWrapper(
    dataSource = h2DataSource,
    databaseType = DBType.H2
)

Kronos.dataSource = { h2Wrapper }
```

H2 的标准双引号标识符由 `SqlDialect.H2` 渲染；upsert 使用 `MERGE ... USING (VALUES ...)`，表结构读取 `INFORMATION_SCHEMA`，`@PrimaryKey(identity = true)` 使用 `GENERATED BY DEFAULT AS IDENTITY`。插入生成 ID 时，结果通过 JDBC generated keys 返回。
...
连接池和 JDBC Driver 推荐使用对应厂商发布的最新稳定版，并按数据库服务端版本和 JDK 选择兼容构件。Kronos 自身依赖示例使用 `0.3.1`。
...
-- H2
MERGE INTO "user" AS "t1" USING (VALUES (...)) AS "t2" ("id", "name") ON (...)
```
...
-- H2 / Oracle / DM8
-- JDBC generated keys are returned by insert and exposed as result.lastInsertId
```
...
`@LogicDelete` 标记删除字段。逻辑删除字段必须使用 `@Default` 声明数据库中的活动值：MySQL、SQLite、H2、SQL Server、Oracle 和 DM8 使用 `@Default("0")`，PostgreSQL 使用 `@Default("false")`。启用后，普通 `delete()` 生成写入删除标记的 `UPDATE`；需要物理删除时使用 `.logic(false)`。
...
- 依赖坐标无法解析：检查 `com.kotlinorm:kronos-core:0.3.1`、`com.kotlinorm:kronos-jdbc-wrapper:0.3.1` 和数据库 driver 的当前稳定版。
