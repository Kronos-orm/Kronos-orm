...
- 充分利用 JVM 生态（驱动、日志、连接池等），并面向未来的 Kotlin 多平台。
- 编译器插件驱动，协程友好，核心路径零反射，性能稳定、GC 压力低。
...
- 易于集成任意技术栈（Spring、Ktor、Vert.x、Solon、Android），下文提供示例仓库。
- 需要精准控制时可使用命名参数的原生 SQL。
- 编译期 Map 映射与元信息 API，KPojo 转换零反射、近乎零开销。

## 开发者文档
- 更完整的架构图、模块内部与深度指南请见[开发者文档入口](./develop-docs/README-zh_CN.md)。
- 适合阅读的人群：
  - 贡献者 / 维护者：希望扩展或修改 Kronos。
  - 高阶使用者：需要理解内部机制与设计取舍。
  - 集成者 / 插件作者：编写日志、JDBC、代码生成等扩展，或将 Kronos 集成到你的平台中。
