...
// Insert a record into table `user` (id = 1, username = "test", age = 18)
...
// Update records in table `user` where `id` is 1, set `age` to 19
User(id = 1, age = 19).update().by { it.id }.execute()

// Delete records from table `user` where `id` is 1
