...
import com.kotlinorm.compiler.core.analyzeAndBuildSqlExpr
import com.kotlinorm.compiler.core.setConditionSqlExprSymbol
 ...
 *
 * Transforms the return statement of a condition lambda into a sqlExpr assignment.
 * @author: Jieyao Lu
 *
 ...
 *
 *    sqlExpr = SqlExpr.Binary(Column("name"), Equal, Parameter("name"))
 */
        ...
        return try {
            with(pluginContext) {
                DeclarationIrBuilder(pluginContext, irFunction.symbol).irBlock {
                    val inner = analyzeAndBuildSqlExpr(irFunction, expression.value, errorReporter)
                    val receiver = irFunction.parameters.extensionReceiver

                    if (inner != null && receiver != null) {
                        +irCall(setConditionSqlExprSymbol).apply {
