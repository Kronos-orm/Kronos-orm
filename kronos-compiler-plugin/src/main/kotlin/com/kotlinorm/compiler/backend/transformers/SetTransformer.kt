...
import com.kotlinorm.compiler.core.isSelectableTypeHintValue
...
class SetTransformer(
        ...
        return if (expression.origin == null) {
            with(pluginContext) {
                DeclarationIrBuilder(pluginContext, irFunction.symbol).irBlock {
                    ...
                    expression.dropSelectableTypeHintAssignments()
            ...
            is IrCall -> {
                ...
                when (element.origin) {
                    IrStatementOrigin.EQ -> {
                        ...
                        if (value != null) {
                            val valueExpression = value.safeSelectableValueExpression()
    ...
    private fun IrBlock.dropSelectableTypeHintAssignments() {
        statements.removeAll { statement ->
            statement is IrCall && statement.origin == IrStatementOrigin.EQ &&
                statement.getValueArgumentSafe(0)?.isSelectableTypeHintValue() == true
        }
        statements.filterIsInstance<IrBlock>()
            .filter { it.origin == null }
            .forEach { statement ->
                statement.dropSelectableTypeHintAssignments()
            }
    }
