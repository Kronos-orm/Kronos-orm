...
fun KotlinBlockBuilderContext.setCriteriaIr(irFunction: IrFunction): IrExpression {
    with(pluginContext) {
        with(builder) {
            return applyIrCall(
                criteriaSetterSymbol, irGet(buildCriteria(irFunction, irFunction.body!!)!!)
            ) {
...
fun KotlinBlockBuilderContext.buildCriteria(
...
fun IrPluginContext.analyzeMinusExpression(irCall: IrCall): Triple<IrClass, IrExpression, List<String>> {
