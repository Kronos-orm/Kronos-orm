...
// Verifies that !exists(query) keeps the deferred selectable query ref and sets the NOT EXISTS flag.
...
fun notExistsWhere(user: NotExistsUser, block: ToFilter<NotExistsUser, Boolean?>): Criteria? {
...
fun box(): String {
    ...
    val criteria = notExistsWhere(NotExistsUser(id = 7)) {
        !exists(NotExistsOrder().select { order -> order.userId })
    }
    val subqueryValue = criteria?.value as? CriteriaSubqueryValue.Exists
    val queryRef = subqueryValue?.query as? KSelectableQueryRef

    val failures = listOfNotNull(
        expect(criteria?.type == ConditionType.SQL) { "type was ${criteria?.type}" },
        expect(criteria?.not == false) { "criteria not flag was ${criteria?.not}" },
        expect(subqueryValue != null) { "value was ${criteria?.value?.let { it::class.qualifiedName }}" },
        expect(subqueryValue?.not == true) { "exists not flag was ${subqueryValue?.not}" },
