...
// Verifies that `[field1, field2] !in query` lowers to row-value NOT IN subquery criteria.
...
fun tupleNotWhere(user: TupleNotUser, block: ToFilter<TupleNotUser, Boolean?>): Criteria? {
...
fun box(): String {
    ...
    val criteria = tupleNotWhere(TupleNotUser(id = 7, status = 1)) {
    ...
    val failures = listOfNotNull(
        ...
        expect(criteria?.not == true) { "criteria not flag was ${criteria?.not}" },
