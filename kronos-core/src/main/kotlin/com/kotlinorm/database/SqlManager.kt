...
object SqlManager {
    private val DBType.dbSupport
        get() = getDBSupport(this)

    internal fun getDBNameFrom(wrapper: KronosDataSourceWrapper) =
            wrapper.dbType.dbSupport?.getDBNameFromUrl(wrapper)
                    ?: throw UnsupportedDatabaseTypeException(wrapper.dbType)

    fun sqlColumnType(dbType: DBType, type: KColumnType, length: Int, scale: Int) =
            dbType.dbSupport?.getColumnType(type, length, scale)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun getKotlinColumnType(dbType: DBType, sqlType: String, length: Int, scale: Int) =
            dbType.dbSupport?.getKColumnType(sqlType, length, scale)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun columnCreateDefSql(dbType: DBType, column: Field) =
            dbType.dbSupport?.getColumnCreateSql(dbType, column)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun indexCreateDefSql(dbType: DBType, tableName: String, kTableIndex: KTableIndex) =
            dbType.dbSupport?.getIndexCreateSql(dbType, tableName, kTableIndex)
                    ?: throw UnsupportedDatabaseTypeException(dbType)
    ...
    fun getTableCreateSqlList(
            ...
            dbType.dbSupport?.getTableCreateSqlList(
                    dbType,
                    tableName,
                    tableComment,
                    columns,
    ...
    fun getTableExistenceSql(dbType: DBType) =
            dbType.dbSupport?.getTableExistenceSql(dbType)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun getTableTruncateSql(dbType: DBType, tableName: String, restartIdentity: Boolean) =
            dbType.dbSupport?.getTableTruncateSql(dbType, tableName, restartIdentity)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun getTableDropSql(dbType: DBType, tableName: String) =
            dbType.dbSupport?.getTableDropSql(dbType, tableName)
                    ?: throw UnsupportedDatabaseTypeException(dbType)

    fun getTableCommentSql(dataSource: KronosDataSourceWrapper) =
            dataSource.dbType.dbSupport?.getTableCommentSql(dataSource.dbType)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)

    fun getTableColumns(dataSource: KronosDataSourceWrapper, tableName: String) =
            dataSource.dbType.dbSupport?.getTableColumns(dataSource, tableName)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)
    ...
    fun getTableIndexes(
            ...
            dataSource.dbType.dbSupport?.getTableIndexes(dataSource, tableName)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)
    ...
    fun getTableSyncSqlList(
            ...
            dataSource.dbType.dbSupport?.getTableSyncSqlList(
                    dataSource,
                    tableName,
    ...
    fun getOnConflictSql(dataSource: KronosDataSourceWrapper, conflictResolver: ConflictResolver) =
    ...
    fun getSelectSql(dataSource: KronosDataSourceWrapper, select: SelectStatement) =
            dataSource.dbType.dbSupport?.getSelectSql(dataSource, select)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)

    fun getInsertSql(dataSource: KronosDataSourceWrapper, insert: InsertStatement) =
            dataSource.dbType.dbSupport?.getInsertSql(dataSource, insert)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)

    fun getUpdateSql(dataSource: KronosDataSourceWrapper, update: UpdateStatement) =
            dataSource.dbType.dbSupport?.getUpdateSql(dataSource, update)
                    ?: throw UnsupportedDatabaseTypeException(dataSource.dbType)

    fun getDeleteSql(dataSource: KronosDataSourceWrapper, delete: DeleteStatement) =
