...
object MssqlSupport : DatabasesSupport {
    ...
    /** SQL Server-specific SQL renderer for AST-based SQL generation. */
    override val renderer: SqlRenderer = MssqlSqlRenderer()
    ...
    override fun getTableExistenceSql(dbType: DBType) =
            "select count(1) from sys.objects where name = :tableName"
    ...
    override fun getTableSyncSqlList(
        ...
        return indexes.toDelete.map { "DROP INDEX [${it.name}] ON [dbo].[$tableName]" } +
            ...
            """.trimWhitespace()
                ...
                indexes.toAdd.map { getIndexCreateSql(dbType, tableName, it) }
    }
