...
object MysqlSupport : DatabasesSupport {
    ...
    /** MySQL-specific SQL renderer for AST-based SQL generation. */
    override val renderer: SqlRenderer = MysqlSqlRenderer()
    ...
    override fun getOnConflictSql(conflictResolver: ConflictResolver): String {
        ...
        return "INSERT INTO ${quote(tableName)} (${toInsertFields.joinToString { quote(it.columnName) }}) " +
                "VALUES (${
            toInsertFields.joinToString(
                ", "
            ) { ":${it.name}" }
        }) " +
                "ON DUPLICATE KEY UPDATE ${onFields.joinToString(", ") { "${quote(it.columnName)} = :${it.name}" }}"
    }
}
