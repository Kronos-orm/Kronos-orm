...
object OracleSupport : DatabasesSupport {
    ...
    /** Oracle-specific SQL renderer for AST-based SQL generation. */
    override val renderer: SqlRenderer = OracleSqlRenderer()
    ...
    override fun getTableCreateSqlList(
        ...
        return listOfNotNull(
                ...
                *columns.asSequence()
                ...
                else "COMMENT ON TABLE ${quote(tableName)} IS '$tableComment'"
        )
    }
                    ...
                    Field(
                            ...
                            primaryKey =
                                    when {
                                        ...
                                        it["COLUMN_DEFAULT"]?.toString()?.endsWith(".nextval") ==
                                                true -> PrimaryKeyType.IDENTITY
                                        else -> PrimaryKeyType.DEFAULT
                                    },
                            defaultValue =
                                    if (it["COLUMN_DEFAULT"]?.toString()?.endsWith(".nextval") ==
                                                    true
                                    ) {
                                        null
                                    } else {
    ...
    override fun getTableSyncSqlList(
        ...
        syncSqlList.addAll(
                indexes.toDelete.map { "DROP INDEX ${quote(dbName)}.\"${it.name}\"" } +
