...
object PostgresqlSupport : DatabasesSupport {
    ...
    override fun getDBNameFromUrl(wrapper: KronosDataSourceWrapper) =
            wrapper.url.split("//").last().split("/").first()
    ...
    override fun getTableCreateSqlList(
        ...
        return listOfNotNull(
                ...
                *columns.asSequence()
                        .filter { !it.kDoc.isNullOrEmpty() }
                        .map {
                            "COMMENT ON COLUMN \"public\".${quote(tableName)}.${quote(it.columnName)} IS '${it.kDoc}'"
                        }
                        .toList()
                        .toTypedArray(),
                if (tableComment.isNullOrEmpty()) null
                else "COMMENT ON TABLE \"public\".${quote(tableName)} IS '$tableComment'"
        )
    }
                    ...
                    Field(
                            ...
                            primaryKey =
                                    when {
                                        ...
                                        it["column_default"]?.toString()?.startsWith("nextval(") ==
                                                true -> PrimaryKeyType.IDENTITY
                                        else -> PrimaryKeyType.DEFAULT
                                    },
                            // 如果defaultValue =  "('${tableName}_id_seq'::regclass)" 设置成 null
                            defaultValue =
                                    if (it["column_default"]?.toString()?.startsWith("nextval(") ==
                                                    true
                                    ) {
                                        null
                                    } else {
    ...
    override fun getTableIndexes(
        ...
        return dataSource.forList(
                ...
                .map { KTableIndex(it["name"] as String, arrayOf(), "", "") }
    }
    ...
    override fun getTableSyncSqlList(
        ...
        if (originalTableComment.orEmpty() != tableComment.orEmpty()) {
            syncSqlList.add(
                    "COMMENT ON TABLE ${quote("public")}.${quote(tableName)} IS '${tableComment.orEmpty()}'"
            )
        }

        syncSqlList.addAll(
                indexes.toDelete.map { "DROP INDEX ${quote("public")}.${it.name};" } +
                        columns.toAdd.map {
