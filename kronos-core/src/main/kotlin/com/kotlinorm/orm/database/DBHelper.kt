...
object DBHelper {
    ...
    fun convertToSqlColumnType(
        ...
        return getSqlType(type, dbType, length).let {
            when {
                dbType == DBType.Postgres && primaryKey && identity -> " SERIAL PRIMARY KEY"
                dbType == DBType.Postgres && primaryKey -> " $it PRIMARY KEY "
                primaryKey && identity && dbType == DBType.Oracle -> "$it GENERATED ALWAYS AS IDENTITY"
    ...
    // 获取数据库类型对应的SQL类型
    fun getSqlType(type: String, dbType: DBType, length: Int) = when (type) {
        "BIT" -> when (dbType) {
