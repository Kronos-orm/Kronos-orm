...
private const val DerivedJoinAlias = "q"

@PublishedApi
@Suppress("UNCHECKED_CAST")
internal fun <T1 : KPojo, R : KPojo, C : KPojo> SelectFrom<T1, *, *>.selectGeneratedProjection(
    projectionClass: KClass<R>,
    contextClass: KClass<C>,
    fields: ToSelect<T1, Any?> = null
): SelectFrom<T1, R, C> {
    context.projectionKClass = projectionClass
    context.receiverPojo = contextClass.createInstance()
...
fun <T1 : KPojo, T2 : KPojo> T1.join(
    ...
    val row = query.selectedKClass.createInstance()
    return SelectFrom2(this, row).apply {
        val tableName = row.__tableName
        context.derivedJoinQueries[tableName] = query to DerivedJoinAlias
        context.derivedJoinAliasOverrides[tableName] = DerivedJoinAlias
        selectFrom(t1, t2)
    }
}
