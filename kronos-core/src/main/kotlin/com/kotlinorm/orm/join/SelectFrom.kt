 ...
 */
open class SelectFrom<T1 : KPojo>(t1: T1) : KSelectable<T1>(t1) {
    open val t1: T1 = t1
    ...
    fun select(someFields: ToSelect<T1, Any?>) {
        ...
        pojo.afterSelect {
            ...
            selectFields += fields
            selectStatement.setProjectionsFromFields(selectFields)
        }
    }
    ...
    fun having(selectCondition: ToFilter<T1, Boolean?> = null) {
        ...
        pojo.afterFilter {
            ...
            if (criteria == null) return@afterFilter
            selectStatement.setHavingCriteria(criteria)
        }
    }
    ...
    override fun build(wrapper: KronosDataSourceWrapper?): KronosQueryTask {
        ...
        renderedParams.forEach { (key, value) ->
