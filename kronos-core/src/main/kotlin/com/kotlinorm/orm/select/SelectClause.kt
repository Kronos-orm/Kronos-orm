...
class SelectClause<T : KPojo>(
        ...
        if (setSelectFields != null) {
            pojo.afterSelect {
                ...
                if (fieldsSet.isNotEmpty()) {
                    selectAll = false
                    // Update statement with select fields directly
                    statement.selectList.clear()
                    statement.selectList.addAll(fieldsToSelectItems(fieldsSet))
                }
            }
        }
    }
    ...
    override fun build(wrapper: KronosDataSourceWrapper?): KronosQueryTask {
        ...
        val finalSelectFields = if (selectAll) allFields else selectFields
