...
class DatabaseStatementsTest {
    ...
    @Test
    fun `H2 registers its statements while reusing the standard dialect`() {
        assertEquals(SqlDialect.H2, SqlManager.dialectOf(DBType.H2))
        assertSame(H2Statements, SqlManager.statementsOf(DBType.H2))
    }

    @Test
    fun `H2 identity columns render in H2 supported order`() {
        val statements = H2Statements.createTable(
            DatabaseCreateTable(
                tableName = "account",
                tableComment = null,
                columns = listOf(
                    statementField("id", KColumnType.INT, primaryKey = PrimaryKeyType.IDENTITY, nullable = false),
                    statementField("name", KColumnType.VARCHAR, length = 64, nullable = false),
                ),
                indexes = emptyList(),
            )
        )
        ...
        assertEquals(
            listOf(
                "CREATE TABLE IF NOT EXISTS \"account\" (\"id\" INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL PRIMARY KEY, \"name\" VARCHAR(64) NOT NULL)"
            ),
            statements.map { it.toSql(SqlDialect.H2) },
        )
    }

    @Test
    fun `H2 maps information schema INTEGER columns to Kronos INT`() {
        val columns = H2Statements.mapColumns(
            "account",
            listOf(
                mapOf<String, Any>(
                    "COLUMN_NAME" to "added_nullable",
        ...
        assertEquals(KColumnType.INT, columns.single().type)
    }
    ...
    fun `generated key fallbacks are defined as dialect queries`() {
        val insert = SqlDmlStatement.Insert(
            table = SqlTable.Ident(
                "app_user.tb_user",
                identifier = SqlIdentifier.of("app_user", "tb_user")
            ),
            columns = listOf(SqlIdentifier.of("name")),
            mode = SqlInsertMode.Values(listOf(listOf(SqlExpr.StringLiteral("Ada"))))
        )
        val generatedKey = Field("user_id")

        val actual = mapOf(
            "mysql" to MysqlStatements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.MySql),
            "postgres" to PostgresqlStatements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.PostgreSql),
            "sqlite" to SqliteStatements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.SQLite),
            "mssql" to MssqlStatements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.SqlServer),
            "oracle" to OracleStatements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.Oracle),
            "dm8" to Dm8Statements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.Oracle),
            "h2" to H2Statements.lastInsertIdFallback(insert, generatedKey)?.toSql(SqlDialect.H2)
        )

        assertEquals(
            mapOf(
                "mysql" to "SELECT LAST_INSERT_ID()",
                "postgres" to "SELECT LASTVAL()",
                "sqlite" to "SELECT last_insert_rowid()",
                "mssql" to "SELECT SCOPE_IDENTITY()",
                "oracle" to null,
                "dm8" to null,
                "h2" to null
            ),
    ...
    @Test
    fun `DM8 reuses the Oracle dialect and statements`() {
        assertEquals(SqlManager.dialectOf(DBType.DM8), SqlDialect.Oracle)
        assertSame(Dm8Statements, SqlManager.statementsOf(DBType.DM8))
    }
    ...
    @Test
    fun `DM8 keeps Oracle DDL conventions with native identity columns`() {
        assertEquals(
            listOf(
                StatementShape("CreateTable", "ACCOUNT", columns = listOf(
                    ColumnShape("ID", "INT IDENTITY(1,1)", false, "Primary", null),
                    ColumnShape("NAME", "VARCHAR2(32)", false, "NotPrimary", "'guest'")
                )),
                StatementShape("CreateIndex", "ACCOUNT", indexName = "idx_account_name", columns = listOf("tenant_id", "name"), unique = true, method = "BTREE"),
                StatementShape("CommentOnColumn", "ACCOUNT", columnName = "ID", comment = "identifier"),
            ...
            Dm8Statements.createTable(createInput()).map { it.toStatementShape() }
        )
    }
