...
class MysqlFunctionTest : MysqlTestBase() {
    ...
    fun testCountWithHaving() {
        val (sql, _) = user.select { [f.count(1).alias("count"), it.gender] }
            ...
            .having { f.count(1) > 5 }
            .build()
        assertEquals("SELECT COUNT(1) AS count, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING COUNT(1) > :countMin", sql)
    }
    ...
    fun testSumWithHaving() {
        val (sql, _) = user.select { [f.sum(it.score).alias("sum"), it.gender] }
            ...
            .build()
        assertEquals("SELECT SUM(`score`) AS sum, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING SUM(`score`) > :sumMin", sql)
    }
    ...
    fun testAvgWithHaving() {
        val (sql, _) = user.select { [f.avg(it.score).alias("avg"), it.gender] }
            ...
            .build()
        assertEquals("SELECT AVG(`score`) AS avg, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING AVG(`score`) > :avgMin", sql)
    }
    ...
    fun testMaxWithHaving() {
        val (sql, _) = user.select { [f.max(it.score).alias("max"), it.gender] }
            ...
            .build()
        assertEquals("SELECT MAX(`score`) AS max, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING MAX(`score`) > :maxMin", sql)
    }
    ...
    fun testMinWithHaving() {
        val (sql, _) = user.select { [f.min(it.score).alias("min"), it.gender] }
            ...
            .build()
        assertEquals("SELECT MIN(`score`) AS min, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING MIN(`score`) < :minMax", sql)
    }
    ...
    fun testMultipleAggregationsWithGroupByAndHaving() {
        val (sql, _) = user.select {
            ...
            .build()
        assertEquals("SELECT COUNT(1) AS count, SUM(`score`) AS sum, AVG(`score`) AS avg, `gender` FROM `tb_user` WHERE `deleted` = 0 GROUP BY `gender` HAVING COUNT(1) > :countMin AND AVG(`score`) > :avgMin", sql)
    }
}
