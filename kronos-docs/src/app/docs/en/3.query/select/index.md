...
`page(pageIndex, pageSize)` creates an offset page; page indexes start from 1. Add `withTotal()` after `page(...)` when the result also needs a total count.
...
The total-count form returns `PageResult<T>`.
