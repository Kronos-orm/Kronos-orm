...
Without `strictMode` turned on (see: {{$.keyword("configuration/global-config", ["Global Configuration", "Turn off Smart Value Transformations"])}}), if the Map's key-value pairs don't match the type of the KPojo object, Kronos will pass ` ValueTransformer` to implement the type conversion.
...
(Datetime will be converted to the default format {{ $.keyword("configuration/global-config", ["Global Configuration", "Default Date Time Format"]) }}))
