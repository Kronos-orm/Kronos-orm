...
{{ $.annotation("Cascade") }} annotations support setting strategies for cascading delete operations, please refer to：{{ $.keyword("advanced/cascade-delete-action", ["Cascade delete strategy"]) }}。
...
data class Child(
    ...
    @Cascade(
        ...
        defaultValue = [Cascade.RESERVED, "empty"]
    )
...
```kotlin
KPojo.delete().cascade(enabled = false).execute()
```
...
Deleting a User or Role entity can perform cascade deletion. According to the cascade deletion strategy defined by the annotation, the cascade deletion will automatically delete the associated table records.
