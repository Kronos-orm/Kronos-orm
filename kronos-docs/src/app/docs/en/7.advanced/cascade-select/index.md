...
When defining the `KPojo` class, use `@Ignore([IgnoreAction.CASCADE_SELECT])` on a relationship property to skip that property during cascade select.

```kotlin group="Cascade select ignore" name="kotlin" icon="kotlin" {12}
@Table("company")
data class Company(
    @PrimaryKey
    var id: Int? = null,
...
The Employee rows are returned.
The company property is not loaded by cascade select for this query.
```

For all `IgnoreAction` values, see {{ $.keyword("mapping/annotations", ["annotation configuration", "Ignore fields"]) }}.
...
Cascade query does not limit the level and direction of cascade relationship by default. If your cascade relationship is very deep, please pay attention to {{ $.keyword("advanced/cascade-select", ["turn off cascade query"]) }} when querying, or only {{ $.keyword("advanced/cascade-select", ["partially turn on cascade query"]) }} to ensure that you will not **query data you don't need**.
