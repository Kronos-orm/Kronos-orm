...
{{ $.annotation("Cascade") }}注解支持设置级联删除操作的策略，请参考：{{ $.keyword("advanced/cascade-delete-action", ["级联删除策略"]) }}。
...
通过`manyToMany`委托，Kronos简化多对多关系为一对多关系，并为您自动创建关联表记录，详见：{{$.keyword("advanced/cascade", ["级联关系定义", "使用委托实现级联多对多跨中间表关系"] )}}。
...
删除`User`或`Role`实体都可以进行级联删除，根据注解定义的级联删除策略，级联删除会自动删除关联表记录。
