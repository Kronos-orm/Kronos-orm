...
`DataGuardPlugin` 是通过任务事件插件机制实现的内置保护能力。自增主键 ID 的读取属于 insert 执行流程。
...
- doBeforeAction: 在操作任务执行前触发，支持`create`、`drop`、`alter`、`truncate`、`insert`、`update`、`delete`和`onConflict()` upsert。
- doAfterAction: 在操作任务执行后触发，支持`create`、`drop`、`alter`、`truncate`、`insert`、`update`、`delete`和`onConflict()` upsert。

> **Note**
> 为什么没有单独的`doBeforeUpsert`事件？
>
> 任务事件插件只有 query 回调和 action 回调。普通 `upsert()` 会先触发匹配查询，再触发 insert 或 update 操作分支；`onConflict()` upsert 会作为 `operationType = KOperationType.UPSERT` 的 action 任务触发。
