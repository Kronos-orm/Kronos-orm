...
class KronosProjectionDocumentationTargetProvider : DocumentationTargetProvider, PsiDocumentationTargetProvider {
    ...
    private fun documentationTargets(element: PsiElement?, offset: Int): List<DocumentationTarget> {
        ...
        if (property != null) {
            ...
            val fields = projectionType.findCanonicalFields() ?: return emptyList()
            return listOf(
        ...
        if (referenceProjectionType != null && reference.getReferencedName() != "it") {
            val fields = referenceProjectionType.findCanonicalFields() ?: return emptyList()
            return listOf(
                KronosProjectionDocumentationTarget(
                    reference,
        ...
        if (expressionProjectionType != null && reference.getReferencedName() != "it") {
            val fields = expressionProjectionType.findCanonicalFields() ?: return emptyList()
            return listOf(
                KronosProjectionDocumentationTarget(
                    reference,
                    reference.getReferencedName(),
...
class KronosProjectionDocumentationLinkHandler : DocumentationLinkHandler {
    override fun resolveLink(target: DocumentationTarget, url: String): LinkResolveResult? =
        KronosIdeaSafe.guard("projection documentation link", null) {
            ...
            val fields = KronosProjectionIdeBridge.read()
                .canonicalProjectionClassDeclaration(className)
                ?.fields
                ?: return@guard null
...
private fun KtExpression.projectionTypeFromQualifiedReceiver(): KronosProjectionType? {
    ...
    val subjectType = receiverType.classFqName + if (nullable) "?" else ""
    return KronosProjectionType(receiverType.classFqName, subjectType)
}
...
private fun KronosProjectionType.findCanonicalFields(): List<KronosIdeProjectionField>? =
