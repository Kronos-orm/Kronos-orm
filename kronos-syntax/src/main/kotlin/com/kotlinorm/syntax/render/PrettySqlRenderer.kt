...
class PrettySqlRenderer(
    ...
    private fun renderPrettySelect(query: SqlQuery.Select): String = buildString {
        ...
        if (query.window.isNotEmpty()) {
            append("\nWINDOW\n")
            append(query.window.joinToString(",\n") { "${renderIndent(1)}${compactRenderer.renderWindowItemForPretty(it)}" })
        }
        query.qualify?.let {
            append("\nQUALIFY\n")
            append("${renderIndent(1)}${renderPredicatePretty(it)}")
    ...
    private fun renderPrettyUpdate(statement: SqlDmlStatement.Update): String = buildString {
        ...
        append(statement.setPairs.joinToString(",\n") {
            "${renderIndent(1)}${renderAssignmentTarget(it.target)} = ${renderExprPretty(it.value)}"
        })
        statement.where?.let {
            append("\nWHERE\n")
            append("${renderIndent(1)}${renderPredicatePretty(it)}")
    ...
    private fun renderPrettyMergeUpsert(statement: SqlDmlStatement.Upsert): String = buildString {
        ...
        append(statement.values.zip(statement.columns).joinToString(",\n") { (value, column) ->
            "${renderIndent(1)}${renderExprPretty(value)} AS ${renderIdentifier(column)}"
        })
        append("\n) ")
    ...
    private fun renderPrettyMysqlUpsert(statement: SqlDmlStatement.Upsert): String = buildString {
        ...
        when (val action = statement.action) {
            SqlUpsertAction.DoNothing -> {
                ...
                append("${renderIndent(1)}${renderIdentifier(column)} = ${renderIdentifier(column)}")
            }
            is SqlUpsertAction.Update -> {
                append(action.setPairs.joinToString(",\n") {
                    val value = when (val expr = it.value) {
                        is SqlExpr.ExcludedColumn -> "VALUES (${renderIdentifier(expr.identifier)})"
                        else -> renderExprPretty(expr)
                    }
                    "${renderIndent(1)}${renderAssignmentTarget(it.target)} = $value"
                })
            }
        }
        statement.returning?.let {
    ...
    private fun renderColumnList(columns: List<SqlIdentifier>): String =
        columns.joinToString(", ", "(", ")") { renderIdentifier(it) }

    private fun renderQualifiedAssignmentTarget(
        target: com.kotlinorm.syntax.statement.SqlAssignmentTarget,
    ...
    private fun SqlRenderer.renderWindowItemForPretty(item: com.kotlinorm.syntax.expr.SqlWindowItem): String =
        renderQuery(SqlQuery.Select(window = listOf(item))).removePrefix("SELECT * WINDOW ")

    private fun renderPrettyMergeSourceExpr(expr: SqlExpr): String = when (expr) {
