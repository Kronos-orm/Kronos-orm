...
open class StandardSqlRenderer(
    ...
    override fun renderExpr(expr: SqlExpr): String = when (expr) {
        ...
        is SqlExpr.Window -> renderWindowExpr(expr)
        ...
        is SqlExpr.Function -> renderFunction(expr)
        is SqlExpr.ExcludedColumn -> "EXCLUDED.${renderIdentifier(expr.identifier)}"
        is SqlExpr.SourceColumn -> renderQualifiedIdentifier(expr.qualifier, expr.identifier)
    ...
    protected open fun renderDml(statement: SqlDmlStatement): String = when (statement) {
        ...
        is SqlDmlStatement.Truncate -> buildString {
            append("TRUNCATE TABLE ${renderTable(statement.table)}")
            if (statement.restartIdentity) append(" RESTART IDENTITY")
        }
        is SqlDmlStatement.Upsert -> renderUpsert(statement)
    }

    protected open fun renderDdl(statement: SqlDdlStatement): String = when (statement) {
        ...
        is SqlDdlStatement.CreateTableAsSelect -> {
            "CREATE TABLE ${if (statement.ifNotExists) "IF NOT EXISTS " else ""}${renderIdentifier(statement.tableName)} AS ${renderQuery(statement.query)}"
        }
        is SqlDdlStatement.AlterTable.AddColumn -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} ADD COLUMN ${renderColumnDefinition(statement.column)}"
        }
        is SqlDdlStatement.AlterTable.DropColumn -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} DROP COLUMN ${renderIdentifier(statement.columnName)}"
        }
        ...
        is SqlDdlStatement.AlterTable.RenameColumn -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} RENAME COLUMN ${renderIdentifier(statement.oldName)} TO ${renderIdentifier(statement.newName)}"
        }
        is SqlDdlStatement.AlterTable.RenameTable -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} RENAME TO ${renderIdentifier(statement.newName)}"
        }
        is SqlDdlStatement.AlterTable.AlterColumnDefault -> {
            val action = statement.defaultValue?.let { "SET DEFAULT ${renderExpr(it)}" } ?: "DROP DEFAULT"
            "ALTER TABLE ${renderIdentifier(statement.tableName)} ALTER COLUMN ${renderIdentifier(statement.columnName)} $action"
        }
        is SqlDdlStatement.AlterTable.AlterColumnNullable -> {
            val action = if (statement.nullable) "DROP NOT NULL" else "SET NOT NULL"
            "ALTER TABLE ${renderIdentifier(statement.tableName)} ALTER COLUMN ${renderIdentifier(statement.columnName)} $action"
        }
        is SqlDdlStatement.AlterTable.SetTableComment -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} COMMENT = ${renderCommentValue(statement.comment)}"
        }
        is SqlDdlStatement.DropTable -> {
            "DROP TABLE ${if (statement.ifExists) "IF EXISTS " else ""}${renderIdentifier(statement.tableName)}"
        }
        is SqlDdlStatement.CreateIndex -> {
            buildString {
                ...
                append(" INDEX")
                if (statement.concurrently) append(" CONCURRENTLY")
                if (statement.ifNotExists) append(" IF NOT EXISTS")
                append(" ${renderIdentifier(statement.indexName)} ON ${renderIdentifier(statement.tableName)}")
                statement.method?.let { append(" USING $it") }
                append(statement.columns.joinToString(", ", " (", ")") { renderIdentifier(it) })
            }
        }
        is SqlDdlStatement.DropIndex -> buildString {
            append("DROP INDEX")
            if (statement.concurrently) append(" CONCURRENTLY")
            if (statement.ifExists) append(" IF EXISTS")
            append(" ${renderIdentifier(statement.indexName)}")
            statement.tableName?.let { append(" ON ${renderIdentifier(it)}") }
        }
        is SqlDdlStatement.AddConstraint -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} ADD ${renderTableConstraint(statement.constraint)}"
        }
        is SqlDdlStatement.DropConstraint -> {
            "ALTER TABLE ${renderIdentifier(statement.tableName)} DROP CONSTRAINT ${renderIdentifier(statement.constraintName)}"
        }
        is SqlDdlStatement.CommentOnTable -> {
            "COMMENT ON TABLE ${renderIdentifier(statement.tableName)} IS ${renderCommentValue(statement.comment)}"
        }
        is SqlDdlStatement.CommentOnColumn -> {
            "COMMENT ON COLUMN ${renderCommentColumnTarget(statement.tableName, statement.columnName)} IS ${renderCommentValue(statement.comment)}"
        }
    ...
    protected open fun renderUpsert(statement: SqlDmlStatement.Upsert): String = buildString {
        ...
        append(statement.values.zip(statement.columns).joinToString(", ") { (value, column) ->
            "${renderExpr(value)} AS ${renderIdentifier(column)}"
        })
    ...
    private fun renderColumn(expr: SqlExpr.Column): String =
        renderQualifiedIdentifier(expr.qualifier, expr.identifier)
    ...
    private fun renderBinaryOperand(child: SqlExpr, parent: SqlBinaryOperator, isRight: Boolean): String {
        ...
        val needsParens = when {
            ...
            childPrecedence > parent.precedence -> (parent === SqlBinaryOperator.Or && child is SqlExpr.Binary && child.operator === SqlBinaryOperator.And) ||
                (parent.isComparison() && requiresComparisonOperandParens(child))
    ...
    private fun SqlBinaryOperator.isAssociativeWith(child: SqlBinaryOperator): Boolean =
        (this === SqlBinaryOperator.And && child === SqlBinaryOperator.And) ||
            (this === SqlBinaryOperator.Or && child === SqlBinaryOperator.Or)

    private fun SqlBinaryOperator.isComparison(): Boolean = when (this) {
        ...
        SqlBinaryOperator.NotRegexp -> true
        else -> false
    }

    protected open fun requiresComparisonOperandParens(expr: SqlExpr): Boolean =
        expr.isArithmeticBinary()

    private fun SqlExpr.isArithmeticBinary(): Boolean =
        this is SqlExpr.Binary && when (operator) {
            ...
            SqlBinaryOperator.BitwiseRightShift -> true
            else -> false
        }
    ...
    private fun renderWindowExpr(expr: SqlExpr.Window): String {
        val window = expr.window
        return if (window.isNameOnly()) {
            "${renderExpr(expr.expr)} OVER ${renderIdentifier(window.existingWindowName!!)}"
        } else {
            "${renderExpr(expr.expr)} OVER (${renderWindow(window)})"
        }
    }
    ...
    private fun renderWindow(window: SqlWindow): String = listOfNotNull(
        window.existingWindowName?.let { renderIdentifier(it) },
    ...
    private fun SqlWindow.isNameOnly(): Boolean =
        existingWindowName != null && partitionBy.isEmpty() && orderBy.isEmpty() && frame == null
    ...
    private fun renderIdentTable(table: SqlTable.Ident): String = buildString {
        append(renderIdentifier(table.identifier))
    ...
    protected open fun renderSelectItem(item: SqlSelectItem): String = when (item) {
        is SqlSelectItem.Asterisk -> item.qualifier?.let { "${renderIdentifier(it)}.*" } ?: "*"
        is SqlSelectItem.Expr -> renderSelectItemExpr(item.expr) + (item.alias?.let {
            " AS ${renderSelectItemAlias(it, item.metadata)}"
        } ?: "")
    }

    protected open fun renderSelectItemAlias(alias: String, metadata: SqlSelectItemAliasMetadata?): String =
        if (shouldQuoteSelectItemAlias(metadata)) quoteIdent(alias) else safeUnquotedIdent(alias)

    protected open fun shouldQuoteSelectItemAlias(metadata: SqlSelectItemAliasMetadata?): Boolean =
        when (metadata?.scope) {
            SqlSelectItemSourceScope.Aggregate,
            SqlSelectItemSourceScope.Window -> false
            SqlSelectItemSourceScope.Selected -> true
            else -> metadata?.source != null
        }

    protected open fun renderSelectItemExpr(expr: SqlExpr): String =
        if (expr is SqlExpr.Binary) "(${renderExpr(expr)})" else renderExpr(expr)

    protected open fun renderReturning(returning: SqlReturning): String =
        returning.items.joinToString(", ", "RETURNING ") { renderSelectItem(it) }
    ...
    protected open fun renderLock(lock: SqlLock): String = buildString {
        ...
        if (lock.targets.isNotEmpty()) {
            append(lock.targets.joinToString(", ", " OF ") { renderIdentifier(it) })
        }
        lock.waitMode?.let {
            append(" ")
            append(when (it) {
    ...
    protected open fun renderTableAlias(alias: SqlTableAlias): String = buildString {
        append("AS ${renderIdentifier(alias.identifier)}")
    ...
    protected open fun renderColumnDefinition(column: SqlColumnDefinition): String = buildString {
    ...
    protected open fun renderIndexDefinition(index: SqlIndexDefinition): String = buildString {
        if (index.unique) append("UNIQUE ")
        append("INDEX ${renderIdentifier(index.name)}")
        index.method?.let { append(" USING $it") }
        append(index.columns.joinToString(", ", " (", ")") { renderIdentifier(it) })
    }

    protected open fun renderTableConstraint(constraint: SqlTableConstraint): String = buildString {
        constraint.name?.let { append("CONSTRAINT ${renderIdentifier(it)} ") }
        when (constraint) {
            is SqlTableConstraint.PrimaryKey -> {
                append(constraint.columns.joinToString(", ", "PRIMARY KEY (", ")") { renderIdentifier(it) })
            }
            is SqlTableConstraint.Unique -> {
            ...
            is SqlTableConstraint.ForeignKey -> {
                ...
                append(" REFERENCES ${renderIdentifier(constraint.referencedTable)}")
                append(constraint.referencedColumns.joinToString(", ", " (", ")") { renderIdentifier(it) })
            }
        }
    }

    protected fun renderCommentValue(comment: String?): String =
        comment?.let { renderExpr(SqlExpr.StringLiteral(it)) } ?: "NULL"

    protected fun renderCommentColumnTarget(tableName: SqlIdentifier, columnName: SqlIdentifier): String =
        renderIdentifier(SqlIdentifier.of(tableName.parts + columnName.parts))

    protected open fun renderSqlServerExtendedPropertyComment(statement: SqlDdlStatement.SqlServerExtendedPropertyComment): String = buildString {
        val procedure = when (statement.operation) {
            SqlServerExtendedPropertyOperation.Add -> "sp_addextendedproperty"
            SqlServerExtendedPropertyOperation.Update -> "sp_updateextendedproperty"
            SqlServerExtendedPropertyOperation.Drop -> "sp_dropextendedproperty"
        }
        append("EXEC sys.$procedure ")
        append("@name = ${renderExpr(SqlExpr.StringLiteral(statement.propertyName))}")
        if (statement.operation != SqlServerExtendedPropertyOperation.Drop) {
            append(", @value = ${renderCommentValue(statement.comment)}")
        }
        append(", @level0type = ${renderExpr(SqlExpr.StringLiteral("SCHEMA"))}")
        append(", @level0name = ${renderExpr(SqlExpr.StringLiteral(sqlServerExtendedPropertySchemaName(statement)))}")
        append(", @level1type = ${renderExpr(SqlExpr.StringLiteral("TABLE"))}")
        append(", @level1name = ${renderExpr(SqlExpr.StringLiteral(sqlServerExtendedPropertyTableName(statement)))}")
        statement.columnName?.let {
            append(", @level2type = ${renderExpr(SqlExpr.StringLiteral("COLUMN"))}")
            append(", @level2name = ${renderExpr(SqlExpr.StringLiteral(it.last))}")
        }
    }

    private fun sqlServerExtendedPropertySchemaName(statement: SqlDdlStatement.SqlServerExtendedPropertyComment): String =
        statement.schemaName?.last ?: statement.tableName.parts.dropLast(1).lastOrNull() ?: "dbo"

    private fun sqlServerExtendedPropertyTableName(statement: SqlDdlStatement.SqlServerExtendedPropertyComment): String =
        statement.tableName.last
    ...
    protected open fun renderColumnPosition(position: SqlColumnPosition): String = when (position) {
        SqlColumnPosition.First -> "FIRST"
        is SqlColumnPosition.After -> "AFTER ${renderIdentifier(position.columnName)}"
    }

    protected open fun renderSqlServerDropDefaultConstraint(statement: SqlDdlStatement.SqlServerDropDefaultConstraint): String {
        val schemaName = statement.schemaName?.last ?: statement.tableName.parts.dropLast(1).lastOrNull() ?: "dbo"
        val tableName = statement.tableName.last
        val columnName = statement.columnName.last
        return """
            ...
            SET @ConstraintName = (
                ...
                WHERE s.name = ${renderExpr(SqlExpr.StringLiteral(schemaName))}
                    AND t.name = ${renderExpr(SqlExpr.StringLiteral(tableName))}
                    AND c.name = ${renderExpr(SqlExpr.StringLiteral(columnName))}
            );
            IF @ConstraintName IS NOT NULL
                BEGIN
                    DECLARE @DropStmt NVARCHAR(MAX) = N'ALTER TABLE ${quoteIdent(schemaName)}.${quoteIdent(tableName)} DROP CONSTRAINT ' + QUOTENAME(@ConstraintName);
    ...
    protected fun renderAssignmentTarget(target: SqlAssignmentTarget): String = when (target) {
    ...
    private fun renderFunctionName(name: SqlIdentifier): String =
        name.parts.joinToString(".") { safeUnquotedIdent(it) }

    protected open fun quoteIdent(identifier: String): String =
        dialect.leftQuote + identifier.replace(dialect.rightQuote, dialect.rightQuote + dialect.rightQuote) + dialect.rightQuote

    protected fun renderIdentifier(identifier: SqlIdentifier): String =
        identifier.parts.joinToString(".") { quoteIdent(it) }

    protected fun renderQualifiedIdentifier(qualifier: SqlIdentifier?, identifier: SqlIdentifier): String =
        (qualifier?.parts.orEmpty() + identifier.parts).joinToString(".") { quoteIdent(it) }
