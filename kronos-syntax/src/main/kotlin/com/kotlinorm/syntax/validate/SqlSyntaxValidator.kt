...
object SqlSyntaxValidator {
    ...
    private fun validateNode(node: SqlNode, diagnostics: MutableList<SqlValidationDiagnostic>) {
        when (node) {
            ...
            is SqlWindow -> validateWindow(node, diagnostics)
            is SqlWindowItem -> validateWindowItem(node, diagnostics)
        }
    }
    ...
    private fun validateDml(statement: SqlDmlStatement, diagnostics: MutableList<SqlValidationDiagnostic>) {
        when (statement) {
            is SqlDmlStatement.Insert -> {
                ...
                validateDuplicateNames("insert.columns.duplicate", "INSERT column", statement.columns.map { it.canonical }, diagnostics)
                validateInsertMode(statement.columns, statement.mode, diagnostics)
                statement.returning?.let { validateReturning(it, diagnostics) }
            }
            is SqlDmlStatement.Update -> {
                validateTable(statement.table, diagnostics)
                validateDuplicateNames("update.set.duplicate", "UPDATE SET column", statement.setPairs.map { it.target.canonicalName }, diagnostics)
    ...
    private fun validateDdl(statement: SqlDdlStatement, diagnostics: MutableList<SqlValidationDiagnostic>) {
        when (statement) {
            is SqlDdlStatement.CreateTable -> {
                val columnNames = statement.columns.map { it.name.canonical }
            ...
            is SqlDdlStatement.AlterTable.AlterColumnDefault -> statement.defaultValue?.let { validateExpr(it, diagnostics) }
            ...
            is SqlDdlStatement.CreateIndex -> validateDuplicateNames(
                ...
                "CREATE INDEX column",
                statement.columns.map { it.canonical },
                diagnostics
            )
            is SqlDdlStatement.DropIndex -> {}
            is SqlDdlStatement.AddConstraint -> validateTableConstraint(statement.constraint, diagnostics)
            is SqlDdlStatement.DropConstraint -> {}
            is SqlDdlStatement.CommentOnTable -> {}
            is SqlDdlStatement.CommentOnColumn -> {}
            is SqlDdlStatement.Vacuum -> statement.into?.let { validateExpr(it, diagnostics) }
    ...
    private fun validateQuery(query: SqlQuery, diagnostics: MutableList<SqlValidationDiagnostic>) {
        when (query) {
            is SqlQuery.Select -> {
                ...
                validateDuplicateNames("select.window.duplicate", "WINDOW name", query.window.map { it.name.canonical }, diagnostics)
                query.window.forEach { validateWindowItem(it, diagnostics) }
                query.qualify?.let { validateExpr(it, diagnostics) }
    ...
    private fun validateInsertMode(
        columns: List<com.kotlinorm.syntax.SqlIdentifier>,
        mode: SqlInsertMode,
    ...
    private fun validateUpsert(
        ...
        validateTable(statement.table, diagnostics)
        validateDuplicateNames("upsert.columns.duplicate", "UPSERT column", statement.columns.map { it.canonical }, diagnostics)
        validateDuplicateNames("upsert.primary.duplicate", "UPSERT primary key", statement.primaryKeys.map { it.canonical }, diagnostics)
        val updateAction = statement.action as? com.kotlinorm.syntax.statement.SqlUpsertAction.Update
        validateDuplicateNames("upsert.update.duplicate", "UPSERT update column", updateAction?.setPairs?.map { it.target.canonicalName }.orEmpty(), diagnostics)
        ...
        if (updateAction?.setPairs?.isEmpty() == true) {
            diagnostics.error("upsert.update.empty", "UPSERT requires at least one update column.")
        }
        val columnSet = statement.columns.map { it.canonical }.toSet()
        val targetColumns = statement.conflictTarget.columns.ifEmpty { statement.primaryKeys }
        if (targetColumns.isEmpty() && statement.conflictTarget.constraintName == null) {
            diagnostics.error("upsert.target.empty", "UPSERT requires conflict target columns or a constraint name.")
        }
        val updateTargets = updateAction?.setPairs?.map { it.target.canonicalName }.orEmpty()
        (targetColumns.map { it.canonical } + updateTargets).forEach { column ->
            if (column !in columnSet) {
                diagnostics.error("upsert.column.unknown", "UPSERT references column `$column` that is not present in the insert columns.")
            }
        }
        statement.conflictTarget.where?.let { validateExpr(it, diagnostics) }
        updateAction?.setPairs?.forEach { validateExpr(it.value, diagnostics) }
        updateAction?.where?.let { validateExpr(it, diagnostics) }
        statement.returning?.let { validateReturning(it, diagnostics) }
        statement.values.forEach { validateExpr(it, diagnostics) }
    }

    private fun validateReturning(
        returning: com.kotlinorm.syntax.statement.SqlReturning,
        diagnostics: MutableList<SqlValidationDiagnostic>
    ) {
        returning.items.forEach {
            if (it is SqlSelectItem.Expr) validateExpr(it.expr, diagnostics)
        }
    }
    ...
    private fun validateIndexDefinition(
        ...
        validateDuplicateNames("ddl.index.column.duplicate", "index column", index.columns.map { it.canonical }, diagnostics)
        index.columns.forEach { column ->
            if (column.canonical !in knownColumns) {
                diagnostics.error("ddl.index.column.unknown", "Index `${index.name.canonical}` references unknown column `${column.canonical}`.")
            }
        }
    }

    private fun validateTableConstraint(
        constraint: com.kotlinorm.syntax.statement.SqlTableConstraint,
        diagnostics: MutableList<SqlValidationDiagnostic>
    ) {
        when (constraint) {
            is com.kotlinorm.syntax.statement.SqlTableConstraint.PrimaryKey ->
                validateDuplicateNames("ddl.constraint.column.duplicate", "PRIMARY KEY column", constraint.columns.map { it.canonical }, diagnostics)
            is com.kotlinorm.syntax.statement.SqlTableConstraint.Unique ->
                validateDuplicateNames("ddl.constraint.column.duplicate", "UNIQUE column", constraint.columns.map { it.canonical }, diagnostics)
            is com.kotlinorm.syntax.statement.SqlTableConstraint.Check -> validateExpr(constraint.condition, diagnostics)
            is com.kotlinorm.syntax.statement.SqlTableConstraint.ForeignKey -> {
                validateDuplicateNames("ddl.constraint.column.duplicate", "FOREIGN KEY column", constraint.columns.map { it.canonical }, diagnostics)
                validateDuplicateNames("ddl.constraint.reference.duplicate", "FOREIGN KEY referenced column", constraint.referencedColumns.map { it.canonical }, diagnostics)
            }
        }
    }
    ...
    private fun validateWindowItem(item: SqlWindowItem, diagnostics: MutableList<SqlValidationDiagnostic>) {
        validateWindow(item.window, diagnostics)
    }
    ...
    private val SqlAssignmentTarget.canonicalName: String
        get() = when (this) {
            is SqlAssignmentTarget.Column -> (qualifier?.parts.orEmpty() + identifier.parts).joinToString(".")
        }
