...
class QueryRendererTest {
    ...
    fun rendersWindowFunctions() {
        val expression = SqlExpr.Window(
            expr = SqlExpr.Function(name = id("ROW_NUMBER")),
            window = SqlWindow(
    ...
    @Test
    fun rendersSelectLevelWindowAndQualify() {
        val query = SqlQuery.Select(
            select = listOf(
                SqlSelectItem.Expr(col("u", "id")),
                SqlSelectItem.Expr(
                    SqlExpr.Window(
                        ...
                        SqlWindow(existingWindowName = id("recent_window"))
                    ),
                    alias = "rn"
                )
            ),
            ...
            qualify = SqlExpr.Binary(col("rn"), SqlBinaryOperator.LessThanEqual, num("3")),
            orderBy = listOf(SqlOrderingItem(col("rn"), SqlOrdering.Asc))
        )

        assertEquals(
            """SELECT "u"."id", ROW_NUMBER() OVER "recent_window" AS rn FROM "user" AS "u" WINDOW "recent_window" AS (PARTITION BY "u"."tenant_id" ORDER BY "u"."created_at" DESC) QUALIFY "rn" <= 3 ORDER BY "rn" ASC""",
            query.toSql()
        )
    }
