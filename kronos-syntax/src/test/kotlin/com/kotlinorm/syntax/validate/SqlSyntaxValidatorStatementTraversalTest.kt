...
class SqlSyntaxValidatorStatementTraversalTest {
    ...
    fun traversesDmlAndDdlStatementBranches() {
        ...
        val statements = listOf(
            ...
            SqlDmlStatement.Insert(table("copy"), columns = cols("id"), mode = SqlInsertMode.Subquery(source)),
            SqlDdlStatement.CreateTableAsSelect(id("copy"), source),
            SqlDdlStatement.AlterTable.AddColumn(id("user"), SqlColumnDefinition(id("nickname"), SqlType.Varchar(32), defaultValue = str("n/a"))),
            SqlDdlStatement.AlterTable.ModifyColumn(id("user"), SqlColumnDefinition(id("nickname"), SqlType.Varchar(128))),
            SqlDdlStatement.AlterTable.DropColumn(id("user"), id("nickname")),
            SqlDdlStatement.DropTable(id("old_user"), ifExists = true),
            SqlDdlStatement.CreateIndex(id("idx_user_name"), id("user"), cols("name", "name")),
    ...
    fun reportsSetFetchWithTiesAndEmptyLimitWarnings() {
        ...
        assertEquals(
            setOf(
                "set.fetch.with.ties.without.order",
                "select.limit.empty"
            ),
            codes
        )
